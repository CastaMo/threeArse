...
module.exports = function() {
    ...
    router.post("/task/start", function(req, res, next) {
        ...
        var avatar_timestamp = +new Date,
            emotion_timestamp = +new Date;

        var Key1 = `avatar/${avatar_timestamp}_1_${img1_base64.length}.jpeg`,
            Key2 = `emotion/${emotion_timestamp}_2_${img2_base64.length}.jpeg`;

        var temp_avatar_path = path.join(__dirname, `../temp/avatar/${avatar_timestamp}.jpeg`),
            temp_emotion_path = path.join(__dirname, `../temp/emotion/${emotion_timestamp}.jpeg`),
            output_avatar_path = path.join(__dirname, `../store/avatar/${avatar_timestamp}.jpeg`),
            output_emotion_path = path.join(__dirname, `../store/emotion/${emotion_timestamp}.jpeg`);
        ...
        ep.on("write_emotion", function() {
            console.log("finish write_emotion");
            common.transferEmotion([
                temp_emotion_path,
                output_emotion_path,
                moment(time).format("hh:mm:ss")
            ], ep.done("transfer_emotion"));
        });
        ...
        ep.on("upload_server", function(httpResponse, body) {
            ...
            fs.rename(output_avatar_path, path.join(__dirname, `../store/avatar/${number}_${moment(time).format("YYYYMMDD")}.jpeg`), function(err) {
                if (!err) {
                    console.log("finish rename avatar");
                }
            });

            fs.rename(output_emotion_path, path.join(__dirname, `../store/emotion/${number}_${moment(time).format("YYYYMMDD")}.jpeg`), function(err) {
                if (!err) {
                    console.log("finish rename emotion");
                }
            });
        ...
        if (img1_base64.length === 0) {
            ...
            temp_avatar_path = path.join(__dirname, "../temp/default.jpeg");
            ep.done("write_avatar")(null);
        } else {
            var base64 = img1_base64.replace(/^data:image\/\w+;base64,/, "");
            var dataBuffer = new Buffer(base64, "base64");
            fs.writeFile(temp_avatar_path, dataBuffer, ep.done("write_avatar"));
        }


        if (img2_base64.length === 0) {
            // Key2 = "default_avatar/default.png";
            temp_emotion_path = path.join(__dirname, "../temp/default.jpeg");
            ep.done("write_emotion")(null);
        } else {
            var base64 = img2_base64.replace(/^data:image\/\w+;base64,/, "");
            var dataBuffer = new Buffer(base64, "base64");
            fs.writeFile(temp_emotion_path, dataBuffer, ep.done("write_emotion"));
        }
